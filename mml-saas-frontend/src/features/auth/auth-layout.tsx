...
export const AuthLayout: FC<PropsWithChildren> = ({ children }) => {
  ...
  return (
    <div className="grid min-h-svh lg:grid-cols-2">
      ...
      {/* Form Panel */}
      <div className="flex flex-col items-center justify-between p-6 sm:p-12">
        ...
        <footer className="w-full max-w-sm pt-8 mt-8 border-t border-border/50 flex flex-wrap items-center justify-center gap-x-4 gap-y-2 text-xs text-muted-foreground">
          <Link to={lp('/terms')} className="hover:text-foreground transition-colors">
            {t(locale, 'authLayout.footerTerms')}
          </Link>
          <Link to={lp('/privacy')} className="hover:text-foreground transition-colors">
            {t(locale, 'authLayout.footerPrivacy')}
          </Link>
          <Link
            to={lp('/personal-data-policy')}
            className="hover:text-foreground transition-colors"
          >
            {t(locale, 'authLayout.footerDataPolicy')}
          </Link>
          <span className="text-muted-foreground/60">
            {t(locale, 'authLayout.footerCopyright')}
          </span>
        </footer>
      </div>
    </div>
