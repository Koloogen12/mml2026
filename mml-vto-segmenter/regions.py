...
SRC_TABLE = {
    ...
    # Строка ACC защищает то, что человек принёс на себе: очки, серьги, часы.
    # Примерка про одежду, и снимать с покупателя его вещи она не должна.
    #
    # Но защита не работает там, где генерация говорит «здесь фон» или «здесь
    # голая кожа»: значит, аксессуара в этом месте больше нет — рукав стал
    # у́же, рука открылась, вещь другой формы. Оставить там оригинал — это
    # аксессуар, висящий в воздухе. Ровно это Данил нашёл 10.09.2026: широкий
    # золотой манжет рубашки уцелел полоской рядом с рукавом кожаной куртки.
    # Часть, накрытая платьем, стёрлась (ACC x CLOTHES отдаёт генерацию), а
    # часть на фоне осталась.
    #
    # HAIR и FACE остаются за оригиналом: очки на лице и серьги у волос — тот
    # самый случай, ради которого строка и существует.
    ACC:     {BG:_G, HAIR:_O, SKIN:_G, FACE:_O, CLOTHES:_G, ACC:_O},
}
