  ...
  return (
    <div ref={containerRef} className="relative z-20 flex items-center gap-3 mb-8 pb-2 flex-wrap">
        ...
        return (
          <div key={filter.key} className="relative">
            <button
              ...
              className={`text-xs font-light border px-4 py-2 flex items-center gap-1.5 whitespace-nowrap transition-colors ${
                selected.length > 0
            ...
            >
              {filter.name}
              {selected.length > 0 && (
                <span className="bg-foreground text-background text-[0.55rem] w-4 h-4 rounded-full flex items-center justify-center font-medium">
                  {selected.length}
                </span>
              )}
              <ChevronDown
                ...
                {filter.options.map((opt) => {
                  ...
                  return (
                    <button
                      ...
                      className={`w-full text-left px-4 py-2.5 text-xs font-light flex items-center gap-2 transition-colors ${
                        isChecked
                      ...
                      >
                        {isChecked && (
                          <svg width="8" height="6" viewBox="0 0 8 6" fill="none">
                            <path d="M1 3L3 5L7 1" stroke="white" strokeWidth="1.2" />
                          </svg>
                        )}
                      </span>
                      {opt.label}
            ...
            <div className="absolute top-full right-0 mt-1 bg-background border border-border z-50 min-w-[200px] shadow-lg">
              {sort.options.map((opt) => (
                <button
                  key={opt.value}
                  onClick={() => {
                    onSortChange(opt.value === activeSort ? "" : opt.value);
                    setOpenDropdown(null);
                  }}
                  className={`w-full text-left px-4 py-2.5 text-xs font-light transition-colors ${
      ...
      {activeCount > 0 && (
        <button
          onClick={clearAll}
          className="text-xs font-light text-muted-foreground hover:text-foreground flex items-center gap-1 transition-colors ml-1"
        >
          <X className="w-3 h-3" />
          Сбросить
        </button>
...
export function applyFilters<
  T extends { brand: string; name: string; price: string; category?: string; age?: string; color?: string; size?: string }
>(products: T[], filters: ActiveFilters, sort: string): T[] {
  let result = [...products];
  ...
  if (sort === "new") result.sort((a, b) => ((b as any).isNew ? 1 : 0) - ((a as any).isNew ? 1 : 0));
