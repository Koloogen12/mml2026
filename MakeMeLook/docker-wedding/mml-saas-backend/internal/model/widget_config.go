...
type StagesEnabled struct {
	...
	Gender       *bool `json:"gender,omitempty"` // pointer: nil (missing in DB) = enabled
...
// IsGenderEnabled returns true when Gender is nil (legacy records) or explicitly true.
func (s StagesEnabled) IsGenderEnabled() bool {
	return s.Gender == nil || *s.Gender
}
