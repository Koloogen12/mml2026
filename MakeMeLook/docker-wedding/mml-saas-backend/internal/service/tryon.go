...
func (s *TryOnService) processTryOnBackground(tryOnID int64, projectID int, leadID int64, modelPhotoURL string, garments []tryOnGarment, body bodyContext) {
	...
	if err != nil {
		...
		logger.Error("tryon", "background job: Gemini API failed", "try_on_id", tryOnID, "retries", retryCount, "latency_ms", latencyMs, "error", err)
...
func (s *TryOnService) resolveGarments(ctx context.Context, productIDs []string) ([]tryOnGarment, error) {
	...
	for _, pidStr := range productIDs {
		...
		if len(product.Photos) > 0 {
			ph := product.Photos[0]
			// Prefer external URL (e.g. Tilda/CDN) when available — mirrors
			// resolvePhotoURL in storefront service. Falls back to MinIO.
			if ph.ExternalURL != nil && *ph.ExternalURL != "" {
				photoURL = *ph.ExternalURL
...
func (s *TryOnService) callGeminiAPI(ctx context.Context, modelPhotoURL string, garments []tryOnGarment, body bodyContext, aiLog *model.AiApiLog) ([]byte, string, int, error) {
	...
	var lastErr error
	retryCount := 0
	for attempt := 1; attempt <= s.cfg.GeminiMaxRetries; attempt++ {
		imageBytes, ext, err := s.callGeminiOnce(ctx, modelBytes, modelMime, loaded, prompt)
		if err != nil {
			lastErr = err
			if attempt < s.cfg.GeminiMaxRetries {
				retryCount++
	...
	logger.Error("tryon", "Gemini API failed after all retries", "retries", s.cfg.GeminiMaxRetries, "error", lastErr)
	return nil, "", retryCount, fmt.Errorf("Gemini API failed after %d retries: %w", s.cfg.GeminiMaxRetries, lastErr)
}
...
func (s *TryOnService) callGeminiOnce(ctx context.Context, modelBytes []byte, modelMime string, garments []loadedGarment, prompt string) ([]byte, string, error) {
	...
	if s.cfg.AIProxyURL != "" {
		proxyURL, err := url.Parse(s.cfg.AIProxyURL)
		if err != nil {
			return nil, "", fmt.Errorf("parse AI proxy URL: %w", err)
		}
		// Clone the default transport so we keep HTTP/2, TLS handshake
		// timeout, idle conns etc. Gemini genai SDK relies on these.
		transport := http.DefaultTransport.(*http.Transport).Clone()
		scheme := strings.ToLower(proxyURL.Scheme)
		if scheme == "socks5" || scheme == "socks5h" {
			// SOCKS5 dialer (e.g. Shadowsocks sslocal on host 10808).
			// http.ProxyURL does NOT understand socks5 — needs a custom Dialer.
			...
			// When we're using a custom dialer, Proxy must be nil so net/http
			// doesn't try to CONNECT through it as an HTTP proxy too.
			transport.Proxy = nil
		} else {
		...
		logger.Info("tryon", "using AI proxy", "proxy", s.cfg.AIProxyURL, "scheme", scheme)
	}
	...
	result, err := client.Models.GenerateContent(ctx, s.cfg.GeminiTryOnModel, contents, &genai.GenerateContentConfig{
...
// Retries up to 3 times on transient network errors (e.g. HTTP/2 stream errors
// from external CDNs like farfetch) with exponential backoff.
func (s *TryOnService) loadImageBytes(ctx context.Context, imageURL string) ([]byte, string, error) {
	// In docker-compose, MinioPublicURL points at the host port (e.g.
	// http://localhost:9010) which only browsers can reach — from inside the
	// backend container `localhost` is the backend itself. Rewrite any such
	// URL to the internal minio endpoint so server-side downloads work.
	if s.cfg.MinioPublicURL != "" && strings.HasPrefix(imageURL, s.cfg.MinioPublicURL) {
		imageURL = "http://" + s.cfg.MinioEndpoint + strings.TrimPrefix(imageURL, s.cfg.MinioPublicURL)
	}

	var lastErr error
	for attempt := 1; attempt <= 3; attempt++ {
		data, mime, err := s.loadImageBytesOnce(ctx, imageURL)
		if err == nil {
			return data, mime, nil
		}
		lastErr = err
		logger.Warn("tryon", "loadImageBytes transient error, retrying",
			"attempt", attempt, "url", imageURL, "error", err)
		...
		if attempt < 3 {
			select {
			case <-time.After(time.Duration(300*attempt*attempt) * time.Millisecond):
			case <-ctx.Done():
				return nil, "", ctx.Err()
			}
		}
...
func (s *TryOnService) loadImageBytesOnce(ctx context.Context, imageURL string) ([]byte, string, error) {
	req, err := http.NewRequestWithContext(ctx, "GET", imageURL, nil)
	if err != nil {
		return nil, "", err
	}
	req.Header.Set("User-Agent", "MakeMeLook/1.0 (+https://b2b.makemelook.ai)")
	...
	logger.Info("tryon", "loadImageBytes",
		"url", imageURL, "status", resp.StatusCode, "mime", mime, "bytes", len(data))
...
func (s *TryOnService) fitResultToModel(ctx context.Context, modelPhotoURL string, resultBytes []byte, ext string) ([]byte, string) {
	// Same public→internal rewrite as loadImageBytes: in docker the public URL
	// is not reachable from inside the backend container.
	if s.cfg.MinioPublicURL != "" && strings.HasPrefix(modelPhotoURL, s.cfg.MinioPublicURL) {
		modelPhotoURL = "http://" + s.cfg.MinioEndpoint + strings.TrimPrefix(modelPhotoURL, s.cfg.MinioPublicURL)
	}
