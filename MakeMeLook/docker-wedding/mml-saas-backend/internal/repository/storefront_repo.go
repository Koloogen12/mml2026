...
func (r *StorefrontRepository) applyFilters(q *gorm.DB, f StorefrontFilter) *gorm.DB {
	...
	if f.Gender != "" {
		// Unisex items should appear in both "for him" and "for her"
		// listings — treat them as matching any gender filter.
		q = q.Where("(gender = ? OR gender = 'unisex' OR gender IS NULL)", f.Gender)
	}
	if f.Brand != "" {
