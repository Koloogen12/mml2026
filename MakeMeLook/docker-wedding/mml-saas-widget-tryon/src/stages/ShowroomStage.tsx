...
function getBaseSku(sku: string): string {
  // CS-Cart variant format: "8000/5" — strip the variant after "/".
  // Other platforms (NOCONCEPT, Brûler, etc.) use dashes inside the SKU
  // (e.g. "K-001-240") where the full string identifies the model and
  // different colours share the same full SKU. Do NOT strip by "-" —
  // that false-groups completely different products into one "variant set".
...
export const ShowroomStage: FC = () => {
  ...
  // NOTE: clothTypeKeys is now computed further below, after `products` is
  // gender-filtered, so we can also hide empty categories.
  ...
  // Per-product sizes: { [product.id]: "M", ... }. Each garment layer keeps
  // its own size independently so changing jeans to L doesn't overwrite the
  // t-shirt which the user already set to S.
  const [selectedSizes, setSelectedSizes] = useState<Record<string, string>>({});
  const [sizingProduct, setSizingProduct] = useState<WidgetProduct | null>(null);

  // Convenience: read/write the size for whichever product is currently being
  // sized (sizingProduct). Falls back to null when nothing is being sized.
  const selectedSize = sizingProduct ? (selectedSizes[sizingProduct.id] ?? null) : null;
  const setSelectedSize = (size: string | null) => {
    if (!sizingProduct) return;
    setSelectedSizes((prev) => {
      if (!size) {
        const next = { ...prev };
        delete next[sizingProduct.id];
        return next;
      }
      return { ...prev, [sizingProduct.id]: size };
    });
  };
  // Read the persisted size for any product (used by layer badges + carousel).
  const getSizeForProduct = (productId: string) => selectedSizes[productId] ?? null;
  ...
  useEffect(() => {
    ...
    // The shop passes the storefront public_id (UUID) via makeMeLook.open().
    // On CS-Cart/Tilda platforms it's the external id from the host catalog.
    // Match against both so any caller works.
    const target = pendingProductExternalId;
    const product = config.products.find(
      (p) => p.publicId === target || p.externalId === target || p.id === target,
    );
  ...
  // Show only products matching the user's selected gender. Treat `null`
  // and `unisex` as wildcards so streetwear/Brûler items show up for both
  // male and female users. Mirrors the storefront repo filter in the Go
  // backend (see applyFilters in storefront_repo.go).
  const userGender = bodyParams.gender;
  ...
  // Cloth-type tabs (shown in the left rail of the showroom). Filter out
  // categories disabled in admin config AND any category that has no
  // products for the current gender — otherwise the user taps an empty tab.
  const clothTypeKeys = useMemo(() => {
    const ct = config?.clothTypesEnabled;
    const countsByCategory = products.reduce<Record<string, number>>((acc, p) => {
      if (p.category) acc[p.category] = (acc[p.category] || 0) + 1;
      return acc;
    }, {});
    return ALL_CLOTH_TYPE_KEYS.filter((key) => {
      // Admin toggles
      if (ct && key !== 'tops' && key !== 'accessories') {
        const enabled = ct[key as keyof typeof ct] ?? true;
        if (!enabled) return false;
      }
      // Always hide empty categories. "tops" stays visible regardless since
      // it's the mandatory cloth type for the flow.
      if (key === 'tops') return true;
      return (countsByCategory[key] || 0) > 0;
    });
  }, [config, products]);
  ...
  const handleClothTypeClick = useCallback((key: string) => {
    setActiveClothType((prev) => {
      ...
      // Do NOT auto-select a subcategory tag — products without a subcategory
      // (e.g. wedding / farfetch rows in our demo) would be filtered out.
      // The user can narrow by tapping a tag themselves.
      setActiveTag(null);
      return key;
    });
    setShowSizes(false);
    ...
    for (const product of selectedProducts) {
      try {
        ...
        if (product.externalId && isCSCartSite()) {
          const productSize = getSizeForProduct(product.id);
          const cartProductId =
            productSize && product.sizeVariants?.[productSize]
              ? product.sizeVariants[productSize]
              : product.externalId;
          try {
  ...
  return (
    <div className={`relative w-full h-full overflow-hidden rounded-[24px] bg-white ${isLoading ? 'mml-showroom--image-loading' : ''}`}>
            ...
            >
              ...
              {selected ? (
                <>
                  ...
                  />
                  {/* Size badge on filled layer — per-product */}
                  {getSizeForProduct(selected.id) && (
      ...
      {!isFullview && <div className="mml-showroom__bottom-panel absolute bottom-[16px] left-[16px] right-[16px] flex flex-col justify-end gap-[10px] z-[4]">
        ...
        {activeClothType && (
          <div
            className="flex gap-[8px] overflow-x-auto pb-[4px] scrollbar-hide"
            onWheel={(e) => {
            ...
            {productGroups.map((group) => {
              ...
              return (
                ...
                >
                  ...
                  />
                  {isSelected && (() => {
                    const sp = selectedProducts.find(p => group.variants.some(v => v.id === p.id));
                    const sz = sp ? getSizeForProduct(sp.id) : null;
                    return sz ? (
