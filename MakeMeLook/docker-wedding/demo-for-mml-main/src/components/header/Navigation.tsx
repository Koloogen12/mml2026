...
const Navigation = () => {
  ...
  // Current gender context — derived from the URL so it survives catalog
  // navigation: /category/men → male, /category/kids → kids, otherwise
  // we look at ?gender= query (set when you pick Верх/Низ/... from the
  // inside of Для Него). Default is "female" (main / Для Нее).
  const genderContext: 'female' | 'male' | 'kids' =
    location.pathname.startsWith('/category/men') ? 'male' :
    location.pathname.startsWith('/category/kids') ? 'kids' :
    searchParams.get('gender') === 'male' ? 'male' :
    searchParams.get('gender') === 'kids' ? 'kids' :
    'female';

  // Build a category href that preserves the current gender context.
  const withGender = (slug: string) => {
    if (genderContext === 'female') return `/category/${slug}`;
    return `/category/${slug}?gender=${genderContext}`;
  };
  ...
  const topCategories = [
    { name: "ДЛЯ НЕЕ",     href: "/",              active: genderContext === 'female' },
    { name: "ДЛЯ НЕГО",    href: "/category/men",  active: genderContext === 'male'   },
    { name: "ДЛЯ ДЕТЕЙ",   href: "/category/kids", active: genderContext === 'kids'   },
  ];

  const navItems = [
    { name: "Новинки",             href: withGender("new") },
    { name: "Верх",                href: withGender("tops") },
    { name: "Низ",                 href: withGender("bottoms") },
    { name: "Верхняя одежда",      href: withGender("outerwear") },
    { name: "Аксессуары",          href: withGender("accessories") },
    // Wedding is female-only; always link straight to wedding without gender qs.
