...
const ProductInfo = ({ product }: ProductInfoProps) => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="hidden lg:block">
        <Breadcrumb>
          <BreadcrumbList>
            ...
            <BreadcrumbSeparator />
            <BreadcrumbItem>
              <BreadcrumbLink asChild><Link to={categoryPath}>{categoryLabel}</Link></BreadcrumbLink>
            </BreadcrumbItem>
            <BreadcrumbSeparator />
            <BreadcrumbItem>
      ...
      {/* Brand & title */}
      <div>
        <Link to={categoryPath} className="text-xs font-medium text-muted-foreground uppercase tracking-widest hover:text-foreground transition-colors">
          {product.brand ?? ''}
        </Link>
        <h1 className="text-xl md:text-2xl font-light text-foreground mt-2">
          {product.name}
        </h1>
      ...
      {(product.sizes?.length ?? 0) > 0 && (
        <div>
          <div className="flex justify-between items-center mb-3">
            ...
            <Link to="/about/size-guide" className="text-xs text-muted-foreground hover:text-foreground transition-colors underline">
              Размерная сетка
            </Link>
          </div>
          <div className="flex gap-2 flex-wrap">
            {product.sizes.map((size) => (
              <button
                ...
                className={`px-4 py-2.5 text-sm font-light border transition-all ${
                  selectedSize === size
      ...
      <Button
        ...
        onClick={() => {
          (window as any).makeMeLook?.open({ productId: product.id });
        }}
      >
        <img src={tryonIcon} alt="" className="w-5 h-5" />
        Примерить
      </Button>
