...
const ProductGrid = ({ genderOverride }: ProductGridProps = {}) => {
  ...
  // Gender precedence: prop (ForHim/ForKids pages) > ?gender query > none.
  // This lets nav links from Для Него/Для Детей carry "male"/"kids" into
  // the generic /category/:slug page.
  const genderFromQuery = searchParams.get('gender') || undefined;
  const effectiveGender = genderOverride || genderFromQuery;
              ...
              {Array.from({ length: totalPages }).map((_, i) => (
                <Button
                  ...
                  className={`min-w-8 h-8 hover:bg-transparent text-sm ${
                    i === page ? 'underline font-normal' : 'hover:underline font-light'
                  }`}
                  onClick={() => setPage(i)}
                >
                  {i + 1}
