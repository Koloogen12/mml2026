...
// SoftDelete marks a lead as deleted by setting deleted_at.
func (r *LeadRepository) SoftDelete(ctx context.Context, token uuid.UUID) error {
	now := time.Now()
	return dbErr(r.db.WithContext(ctx).
		Model(&model.Lead{}).
		Where("session_token = ? AND deleted_at IS NULL", token).
...
type LeadListFilter struct {
	...
	Engagement string // funnel stage: engaged|with_photo|with_tryon|bounce|"" (all)
...
func (r *LeadRepository) applyAdminFilters(q *gorm.DB, f LeadListFilter) *gorm.DB {
	...
	// Funnel stages: a lead is created on every widget session, so by default
	// the list is dominated by bounces (visited but never engaged). These
	// filters let admins focus on each stage of the funnel.
	switch f.Engagement {
	case "engaged":
		// Filled body params (entered measurements stage).
		q = q.Where("leads.gender IS NOT NULL")
	case "with_photo":
		q = q.Where("EXISTS (SELECT 1 FROM lead_photos lp WHERE lp.lead_id = leads.id AND lp.deleted_at IS NULL)")
	case "with_tryon":
		q = q.Where("EXISTS (SELECT 1 FROM lead_try_ons lt WHERE lt.lead_id = leads.id AND lt.deleted_at IS NULL)")
	case "bounce":
		// Visited but did nothing — no measurements, no photo, no try-on.
		q = q.Where("leads.gender IS NULL AND NOT EXISTS (SELECT 1 FROM lead_try_ons lt WHERE lt.lead_id = leads.id AND lt.deleted_at IS NULL) AND NOT EXISTS (SELECT 1 FROM lead_photos lp WHERE lp.lead_id = leads.id AND lp.deleted_at IS NULL)")
	}
	return q
}
