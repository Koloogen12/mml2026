...
// isHEIC checks magic bytes for HEIC/HEIF container. The standard ISO BMFF
// container has "ftyp" at offset 4, followed by a 4-byte brand. HEIC brands:
// "heic", "heix", "hevc", "hevx", "mif1" (HEIF), "msf1" (HEIF sequence).
...
// convertHEICtoJPEG decodes HEIC and re-encodes as JPEG (quality 92).
// Returns the JPEG bytes or an error if decode fails.
...
// normalizeJPEGOrientation reads the EXIF Orientation tag on a JPEG and,
// if it's anything other than "no rotation", rotates the pixels to match
// and re-encodes the JPEG WITHOUT the EXIF block.
//
// Why we do this: iPhones photograph in landscape sensor orientation and
// encode the "rotate this on display" hint into EXIF Orientation = 6 (90°
// CW). Browsers / Telegram / Apple Photos all respect the tag. But the
// downstream AI providers we hit (Gemini via CometAPI, Nano Banana via
// fal.ai) read the raw pixel grid and IGNORE EXIF — they see a sideways
// person and generate a sideways try-on result (see 2026-05-14 Malina
// Bonita reports). Normalizing here makes the bytes upright for every
// consumer regardless of EXIF support.
//
// Returns the new bytes on success. Returns (nil, nil) when no rotation
// is needed so the caller can keep the original bytes unchanged. On any
// error (EXIF unreadable, decode fail, etc.) returns (nil, err) — the
// caller should treat that as "use original bytes" rather than aborting
// the upload, because reaching this code with a real JPEG already means
// it's renderable somewhere.
func normalizeJPEGOrientation(data []byte) ([]byte, error) {
	orientation := readJPEGOrientation(data)
	// 1 = no rotation, 0 = tag absent (treat as no rotation).
	if orientation <= 1 {
		return nil, nil
	}
	img, err := jpeg.Decode(bytes.NewReader(data))
	if err != nil {
		return nil, fmt.Errorf("decode jpeg for orientation: %w", err)
	}
	var rotated image.Image
	switch orientation {
	case 2: // mirrored horizontally
		rotated = flipHorizontal(img)
	case 3: // rotated 180
		rotated = rotate180(img)
	case 4: // mirrored vertically
		rotated = flipVertical(img)
	case 5: // rotated 90 CCW + mirrored horizontally
		rotated = flipHorizontal(rotate90CW(img))
	case 6: // rotated 90 CW (most common iPhone portrait)
		rotated = rotate90CW(img)
	case 7: // rotated 90 CW + mirrored horizontally
		rotated = flipHorizontal(rotate270CW(img))
	case 8: // rotated 90 CCW (= 270 CW)
		rotated = rotate270CW(img)
	default:
		return nil, nil
	}
	var buf bytes.Buffer
	...
	logger.Info("widget_session", "normalized JPEG orientation",
		"orientation", orientation, "in_bytes", len(data), "out_bytes", buf.Len())
	return buf.Bytes(), nil
}

// readJPEGOrientation extracts the EXIF Orientation tag from JPEG bytes.
// Returns 1 (no rotation) on any decode failure so callers can blindly
// use the result.
...
// rotate90CW returns a new image rotated 90 degrees clockwise. Used by
// normalizeJPEGOrientation for EXIF orientation = 6 (the iPhone-portrait
// default).
...
// rotate270CW returns a new image rotated 270 degrees clockwise (= 90 CCW).
...
// flipHorizontal mirrors an image along the vertical axis.
...
// flipVertical mirrors an image along the horizontal axis.
...
// detectImageMIME returns the canonical MIME type for the uploaded image.
//
// Strategy (most reliable first):
//  1. Try image.DecodeConfig — actually parses the file header and returns
//     the registered format name ("jpeg", "png", "webp", "gif"). This works
//     even when the multipart parser stripped the magic bytes from
//     http.DetectContentType's view, and it works even when the filename
//     extension is wrong or missing.
//  2. Fall back to filename extension if decoder fails to recognize the
//     bytes (rare — usually means HEIC/AVIF/TIFF/SVG which we don't accept
//     anyway).
//  3. Last resort: net/http content sniffing.
//
// Returns "" if we cannot identify a supported image format. Callers should
// reject the upload in that case rather than guessing — passing
// application/octet-stream to Gemini's vision API blows up with a 400 and
// our validator silently fails open, defeating the whole point of running it.
...
func (h *WidgetSessionHandler) CreateSession(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		case errors.Is(err, service.ErrProjectPaused):
			// 403 — loader.ts treats it like domain_not_allowed: silently
			// drops the cached token and skips widget injection.
			WriteError(w, http.StatusForbidden, "project_paused", "Project is paused")
		default:
...
func (h *WidgetSessionHandler) GetSession(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		case errors.Is(err, service.ErrProjectPaused):
			// 403 — see CreateSession's note. loader.ts removes the
			// cached token and stops showing the widget.
			WriteError(w, http.StatusForbidden, "project_paused", "Project is paused")
		default:
...
func (h *WidgetSessionHandler) UploadPhoto(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		logger.Warn("widget_session", "UploadPhoto: parseFile failed",
			"lead_id", lead.ID, "error", err.Error())
	...
	// Buffer bytes so we can both validate and upload from the same source.
	imgBytes, readErr := io.ReadAll(file)
	if readErr != nil {
		WriteError(w, http.StatusInternalServerError, "internal_error", "Failed to read photo")
		return
	}

	// HEIC (iPhone default) — convert to JPEG before anything else touches
	// the bytes. Gemini's vision API + our image.Decode path don't support
	// HEIC; without this step every iPhone selfie that wasn't pre-converted
	// by the OS would be rejected as "MIME not recognized".
	if isHEIC(imgBytes) {
		jpegBytes, err := convertHEICtoJPEG(imgBytes)
		if err != nil {
			logger.Warn("widget_session", "UploadPhoto: HEIC convert failed",
				"lead_id", lead.ID, "filename", filename, "size", len(imgBytes), "error", err)
			h.tryOnSvc.NotifyUploadError(r.Context(), lead.ProjectID, lead.ID, "heic_convert_failed", err.Error())
			WriteError(w, http.StatusBadRequest, "photo_rejected_invalid_image",
				"Не удалось обработать HEIC-фото. Сохраните его как JPG и загрузите снова.")
			return
		}
		logger.Info("widget_session", "UploadPhoto: HEIC converted to JPEG",
			"lead_id", lead.ID, "heic_size", len(imgBytes), "jpeg_size", len(jpegBytes))
		imgBytes = jpegBytes
		// Update filename so downstream logic / storage suffix is correct.
		filename = strings.TrimSuffix(filename, filepath.Ext(filename)) + ".jpg"
		size = int64(len(imgBytes))
	}

	// Determine MIME by actually decoding the image header. If we can't
	// recognize the file as a supported format at all, reject before even
	// asking Gemini — Gemini's vision API rejects application/octet-stream
	// with a 400 and our validator then fails OPEN, silently letting the
	// bad photo through.
	mime := detectImageMIME(filename, imgBytes)
	if mime == "" {
		var headHex string
		if len(imgBytes) >= 16 {
			headHex = fmt.Sprintf("%x", imgBytes[:16])
		}
		logger.Warn("widget_session", "UploadPhoto: MIME not recognized",
			"lead_id", lead.ID, "filename", filename, "size", size,
			"bytes_read", len(imgBytes), "head_hex", headHex)
		h.tryOnSvc.NotifyUploadError(r.Context(), lead.ProjectID, lead.ID, "mime_not_recognized",
			fmt.Sprintf("filename=%s size=%d head=%s", filename, size, headHex))
		WriteError(w, http.StatusBadRequest, "photo_rejected_invalid_image",
			"Не удалось распознать формат фото. Сохраните его как JPG, PNG или WebP и загрузите снова.")
		return
	}

	// EXIF orientation normalization. iPhone photos shot in portrait are
	// stored as landscape pixels + an EXIF "rotate 90 CW" tag — browsers
	// and Telegram respect it, but Gemini / fal.ai operate on raw pixels
	// and ignore EXIF. Without this step the AI model sees the customer
	// lying sideways, generates a sideways result, and customers see
	// horizontal output as on 2026-05-14 Malina Bonita screenshots.
	//
	// We physically rotate pixels per the EXIF tag and strip EXIF, so
	// every downstream consumer (validator, generator, MinIO, browser)
	// gets the same upright bytes. JPEG only — PNG and WebP very rarely
	// carry orientation tags.
	if mime == "image/jpeg" {
		if rotated, rotErr := normalizeJPEGOrientation(imgBytes); rotErr == nil && rotated != nil {
			imgBytes = rotated
			size = int64(len(imgBytes))
		}
	}

	// MinIO PUT happens BEFORE validation — keeping validation off the hot
	// path matters because CometAPI sometimes takes 10+ seconds to respond.
	// On a slow user connection that already burned 30-90 seconds uploading,
	// adding another 10+ seconds of server-side wait yields the "loading
	// forever" UX Danil flagged. With this ordering, the user gets a 200 as
	// soon as the bytes hit MinIO, and validation runs in a goroutine — if
	// it eventually rejects the photo we soft-delete the lead_photo row and
	// notify in Telegram. The widget still gets the photo URL straight away.
	objectKey, err := h.storageSvc.UploadImage(r.Context(), h.bucket, bytes.NewReader(imgBytes), filename, size, 10*1024*1024, allowedExts)
	if err != nil {
	...
	// SYNC validation gate (Phase 1 strict validation).
	//
	// Previously validation ran in a background goroutine after we'd already
	// returned 201 to the user — they sailed straight to the try-on stage
	// no matter what the validator said. With Pro-class validation
	// (gemini-2.5-pro) we now block the upload: if the photo is rejected
	// (bulky parka, distant framing, selfie close-up, multiple people, …),
	// the widget shows a localised reject screen with the matching
	// reason_code from internal/service/tryon.go::validationPromptText.
	//
	// Force override: the widget re-submits the same upload with
	// ?force=true after the user clicks "Try anyway" on the reject screen.
	// We honour the override, log it via Telegram, and let the generator
	// deal with the consequences (this is rare-path UX, not a bypass we
	// want users discovering on their own).
	...
	if !forceOverride {
		...
		if vErr == nil && validation != nil && !validation.OK {
			...
			if msg == "" {
				msg = "Фото не подходит для примерки."
			}
			h.tryOnSvc.NotifyValidationRejected(r.Context(), projectID, leadID, reason, msg)
			// 422 = well-formed request, semantically unprocessable.
			// We keep the MinIO object so the "Try anyway" path can avoid
			// re-uploading bytes — for now the widget just re-POSTs the
			// same file with force=true (simpler than wiring an
			// accept-pending endpoint, and uploads are <10MB / rare).
			photoURL := h.storageSvc.GetObjectURL(h.bucket, objectKey)
		...
		// Validator either fail-open (unavailable) or accepted. Track both
		// so we can monitor validator coverage in Telegram over time.
		statusLabel := "ok"
		if validation == nil || vErr != nil {
			statusLabel = "fail-open (validator unavailable)"
		}
		h.tryOnSvc.NotifyPhotoUploaded(r.Context(), projectID, leadID, statusLabel, int(size), mime)
	} else {
		// User explicitly chose "Try anyway" after a rejection.
		h.tryOnSvc.NotifyPhotoUploaded(r.Context(), projectID, leadID, "force-override (validator bypassed)", int(size), mime)
	}
...
func (h *WidgetSessionHandler) TryOn(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		case errors.Is(err, service.ErrTooManyGarments):
			WriteError(w, http.StatusBadRequest, "invalid_request", "Maximum 5 products allowed")
...
// RecordConsent handles POST /api/widget/v1/sessions/{token}/consent — logs
// the user's explicit consent to biometric personal data processing under
// Article 11 of Russian Federal Law No. 152-FZ.
func (h *WidgetSessionHandler) RecordConsent(w http.ResponseWriter, r *http.Request) {
	token, ok := h.parseSessionToken(w, r)
	if !ok {
		return
	}
	...
	resp, err := h.leadSvc.RecordConsent(r.Context(), lead, token, &req, r.RemoteAddr, r.UserAgent())
	if err != nil {
...
func renderShareHTML(w http.ResponseWriter, data *dto.ShareViewResponse) {
	...
	// Product list — name (linked to product page when URL present) on the
	// left, price on the right. Falls back to category when no price.
	productListHTML := ""
	for _, p := range data.Products {
		// Name: clickable link when a product URL is present, plain text otherwise.
		nameHTML := html.EscapeString(p.Name)
		if p.URL != nil && *p.URL != "" {
			nameHTML = fmt.Sprintf(`<a href="%s" class="product-name-link" target="_blank" rel="noopener">%s</a>`,
				html.EscapeString(*p.URL), html.EscapeString(p.Name))
		}

		// Right side: price if available, else category (capitalized by CSS).
		rightHTML := ""
		if p.Price != nil && *p.Price > 0 {
			...
			if p.Currency != nil && *p.Currency != "" {
				...
				case "EUR":
					cur = "€"
				default:
					cur = html.EscapeString(*p.Currency)
				}
			}
			// Russian-style thousand separator (space) for readability.
			rightHTML = fmt.Sprintf(`<span class="product-price">%s %s</span>`, formatPrice(*p.Price), cur)
		} else {
			rightHTML = fmt.Sprintf(`<span class="product-cat">%s</span>`, html.EscapeString(p.Category))
		}

		productListHTML += fmt.Sprintf(`<div class="product"><span class="product-name">%s</span>%s</div>`, nameHTML, rightHTML)
	}
...
.product-name{font-size:14px;font-weight:500;flex:1;padding-right:12px;min-width:0}
.product-name-link{color:inherit;text-decoration:none;border-bottom:1px solid transparent;transition:border-color 0.15s}
.product-name-link:hover{border-bottom-color:#1a1a1a}
.product-cat{font-size:12px;color:#888;text-transform:capitalize;flex-shrink:0}
.product-price{font-size:14px;font-weight:600;color:#1a1a1a;flex-shrink:0;white-space:nowrap}
...
<div class="footer">
  Создано с помощью <a href="https://b2b.makemelook.ai/ru" target="_blank">MakeMeLook</a> — AI-стилист для интернет-магазинов
</div>
...
// formatPrice renders a price without trailing .00 and with a non-breaking
// space as a thousands separator (e.g. 14500 → "14 500", 2499.90 → "2 499.90").
func formatPrice(v float64) string {
	// Strip trailing zeros after decimal; keep up to 2 decimal places.
