...
// PUT /api/v1/projects/:id/tryon-provider
func (h *ProjectHandler) UpdateTryonProvider(w http.ResponseWriter, r *http.Request) {
	userID, projectID, ok := parseProjectRequest(w, r)
	if !ok {
		return
	}

	var req dto.UpdateProjectTryonProviderRequest
	if !decodeAndValidate(w, r, &req) {
		return
	}

	resp, err := h.projectService.UpdateTryonProvider(r.Context(), userID, projectID, req)
	if err != nil {
		mapServiceError(w, err)
		return
	}
...
func mapServiceError(w http.ResponseWriter, err error) {
	...
	case errors.Is(err, service.ErrCustomFeedNeedsHTTPS):
		WriteError(w, http.StatusBadRequest, "https_required", "URL фида должен использовать HTTPS")
	case errors.Is(err, service.ErrProjectPaused):
		// 403 — the loader treats this exactly like "domain not allowed":
		// silently hides the widget. Don't change the status code without
		// updating mml-saas-widget-tryon/src/loader.ts in lockstep.
