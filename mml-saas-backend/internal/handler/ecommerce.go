...
// GET /api/v1/projects/{id}/integrations/{storeId}/history?limit=50
func (h *EcommerceHandler) ListSyncHistory(w http.ResponseWriter, r *http.Request) {
	userID, projectID, ok := parseProjectRequest(w, r)
	if !ok {
		return
	}
	...
	// Optional limit query param. Service clamps to a sane range.
	limit := 0
	...
	resp, err := h.ecommerceSvc.ListSyncHistory(r.Context(), userID, projectID, storeID, limit)
	if err != nil {
		mapServiceError(w, err)
		return
	}
