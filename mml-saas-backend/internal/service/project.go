...
// UpdateTryonProvider switches the AI provider used for try-on generation
// on a per-project basis. Atomic with ownership check.
func (s *ProjectService) UpdateTryonProvider(ctx context.Context, userID int, projectID int, req dto.UpdateProjectTryonProviderRequest) (*dto.ProjectResponse, error) {
	provider := model.TryonProvider(req.TryonProvider)
	affected, err := s.repos.Project.Update(ctx, projectID, userID, &repository.UpdateProjectData{
		TryonProvider: &provider,
	})
	if err != nil {
		return nil, fmt.Errorf("update tryon provider: %w", err)
	}
	if affected == 0 {
		return nil, ErrProjectNotFound
	}
...
func (s *ProjectService) projectToDTO(p *model.Project) *dto.ProjectResponse {
	...
	if provider == "" {
		provider = string(model.TryonProviderCometAPIGemini)
	}
