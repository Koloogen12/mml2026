...
func (s *InstallationService) checkDomain(ctx context.Context, projectID int, domain, publicID string) dto.DomainDiagnostics {
	...
	// 1. Fetch HTML page. Defensive against legacy DB rows where `domain`
	// might still contain a scheme/path (we normalize on insert now, but
	// older data lives on) — without this we'd build "https://http://..."
	// and DNS would try to resolve "http" as a hostname.
	host := NormalizeDomainForWhitelist(domain)
	if host == "" {
		host = domain
	}
	pageURL := "https://" + host
