...
type TryOnRequest struct {
	...
	// + shoes + accessories). Service-layer cap and widget-side cap match.
	ProductIDs []string `json:"product_ids" validate:"required,min=1,max=5"`
	// Regenerate bypasses the per-lead cache for (photo + garment combo)
	// and forces a fresh AI render. Default false → if the same combo
	// was rendered before for this lead, the prior result is reused
	// instantly. Wire this from the widget's "Regenerate" button (UI
	// for that is on the roadmap).
...
// --- Consent (biometric personal data processing) ---

type RecordConsentRequest struct {
	PolicyType    string `json:"policy_type" validate:"required,max=50"`
	PolicyVersion string `json:"policy_version" validate:"required,max=50"`
