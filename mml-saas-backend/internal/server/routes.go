...
func (s *Server) routes() http.Handler {
	...
	r.Route("/api/v1", func(r chi.Router) {
		...
		// Waitlist (public, makemelook.ai landing)
		r.Post("/waitlist", s.waitlist.Signup)
		...
		r.Group(func(r chi.Router) {
			...
			r.Put("/projects/{id}/tryon-provider", s.project.UpdateTryonProvider)
			...
			r.Get("/projects/{id}/integrations/{storeId}/history", s.ecommerce.ListSyncHistory)
	...
	// Public storefront API (no auth — used by /shop demo page and similar)
	r.Route("/api/storefront/v1", func(r chi.Router) {
		r.Use(middleware.WidgetCORS())
		r.Get("/{projectPublicId}/products", s.storefront.ListProducts)
		r.Get("/{projectPublicId}/products/{productPublicId}", s.storefront.GetProduct)
		r.Get("/{projectPublicId}/categories", s.storefront.GetCategories)
	})
	...
	r.Route("/api/widget/v1", func(r chi.Router) {
		...
		// Session create (handled inline — paused-check lives inside the service)
		r.Post("/sessions", s.widgetSession.CreateSession)

		// All /sessions/{token}/* routes are kill-switch-guarded: if the
		// project that owns the session is paused, every request is rejected
		// with 403 project_paused. This is the rest of the kill-switch that
		// the explicit checks in CreateSession/GetSession can't cover (those
		// only protect the loader handshake).
		r.Group(func(r chi.Router) {
			r.Use(middleware.WidgetSessionPausedGuard(s.pausedChecker))

			// Sessions (Lead API)
			r.Get("/sessions/{token}", s.widgetSession.GetSession)
			r.Put("/sessions/{token}", s.widgetSession.UpdateSession)
			r.Delete("/sessions/{token}", s.widgetSession.DeleteSession)
			...
			r.Post("/sessions/{token}/consent", s.widgetSession.RecordConsent)
		})
