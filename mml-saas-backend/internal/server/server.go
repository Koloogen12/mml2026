...
type Server struct {
	...
	pausedChecker       middleware.SessionPausedChecker
}
...
func New(cfg *config.Config, repos *repository.Repositories, minioClient *minio.Client, mlr *mailer.Mailer) *Server {
	...
	// Public pre-launch waitlist (makemelook.ai landing). 4800 = queue display base.
	waitlistSvc := service.NewWaitlist(repos, 800)
	...
	// Paused-status checker: resolves project paused state from a session
	// token. Used by WidgetSessionPausedGuard to enforce the admin
	// kill-switch on every /sessions/{token}/* request, not just the
	// initial loader handshake.
	pausedChecker := middleware.SessionPausedChecker(func(ctx context.Context, token uuid.UUID) (bool, error) {
		lead, err := repos.Lead.GetBySessionToken(ctx, token)
		if err != nil {
			return false, err
		}
		if lead == nil {
			// Unknown session — let the downstream handler return 404
			// instead of having middleware mask it.
			return false, nil
		}
		project, err := repos.Project.GetByID(ctx, lead.ProjectID)
		if err != nil {
			return false, err
		}
		if project == nil {
			// Orphaned session (project deleted) — treat as paused so
			// abandoned sessions can't keep generating Gemini calls.
			return true, nil
		}
		return project.IsPaused(), nil
	})
	...
	s := &Server{
		...
		storefront:          handler.NewStorefront(productSvc),
		...
		waitlist:           handler.NewWaitlist(waitlistSvc),
