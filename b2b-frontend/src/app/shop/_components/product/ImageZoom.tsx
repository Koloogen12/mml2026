...
const ImageZoom = ({ images, initialIndex, isOpen, onClose }: ImageZoomProps) => {
  ...
  useEffect(() => {
    const handleEscKey = (event: KeyboardEvent) => { if (event.key === 'Escape') onClose(); };
    if (isOpen) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/90 backdrop-blur-sm animate-fade-in">
      ...
      <Button variant="ghost" size="sm" onClick={onClose} className="absolute top-6 right-6 z-10 hover:bg-transparent text-black border-none p-2">
        <X className="h-8 w-8" />
      </Button>
        ...
        <div className="space-y-4">
          {images.map((image, index) => (
            <div key={index} className="w-full flex justify-center">
              <img src={image} alt={`Product view ${index + 1}`} className="w-full max-w-none object-cover animate-scale-in" />
            </div>
          ))}
        </div>
      </div>
