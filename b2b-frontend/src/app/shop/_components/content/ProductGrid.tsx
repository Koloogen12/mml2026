...
const ProductGrid = () => {
  ...
  const { data, isLoading, isError } = useProducts({ limit: 100, sort: activeSort });

  const filterConfigs: FilterConfig[] = useMemo(() => {
    if (!data?.filters) return [];
    const configs: FilterConfig[] = [];
    if (data.filters.categories.length) {
      configs.push({ name: 'Категория', key: 'category', options: data.filters.categories.map((c) => ({ label: c, value: c })) });
    }
    if (data.filters.brands.length) {
      configs.push({ name: 'Бренд', key: 'brand', options: data.filters.brands.map((b) => ({ label: b, value: b })) });
    }
    if (data.filters.colors.length) {
      configs.push({ name: 'Цвет', key: 'color', options: data.filters.colors.map((c) => ({ label: c, value: c })) });
    }
    return configs;
  }, [data?.filters]);
  ...
  return (
    <section className="w-full mb-16 px-6">
      <CatalogFilters
        ...
        onSortChange={(sort) => setActiveSort((sort as StorefrontListParams['sort']) || 'newest')}
      />
      {filtered.length === 0 ? (
