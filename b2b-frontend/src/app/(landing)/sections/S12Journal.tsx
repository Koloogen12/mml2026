...
export function S12Journal({ v }: { v: LandingVals }) {
  return (
    <section id="journal" data-screen-label="12 Journal" style={{ padding: "200px 24px 0", maxWidth: "1288px", margin: "0 auto" }}>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(320px,1fr))", alignItems: "end", gap: "24px 48px", marginBottom: "40px" }}>
        <div>
          ...
          <h2 style={{ margin: "18px 0 0", fontFamily: "Unbounded,sans-serif", fontWeight: "500", fontSize: "clamp(28px,3vw,44px)", lineHeight: "1.08", letterSpacing: "-.03em", textWrap: "balance" }}>
            Что мы узнаём, пока считаем возвраты
          </h2>
        </div>
        {" "}
        <p style={{ margin: "0", maxWidth: "420px", fontSize: "17px", lineHeight: "1.5", color: "#565E6B", justifySelf: "end" }}>
          Разборы на данных магазинов, с методикой и источниками. Без обещаний процентов — только то, что можно проверить.
        </p>
      </div>
      {" "}
      {v.jLead ? (
      <a className="scp5" href={v.jLead.href} style={{ position: "relative", display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(300px,1fr))", borderRadius: "28px", overflow: "hidden", background: "#0E1014", color: "#fff", textDecoration: "none", minHeight: "440px" }}>
        ...
        {" "}
        <div style={{ position: "relative", padding: "clamp(28px,3.4vw,48px)", display: "flex", flexDirection: "column", justifyContent: "space-between", gap: "28px" }}>
          ...
          {" "}
          <span style={{ display: "inline-flex", alignItems: "center", gap: "12px", fontSize: "16px", fontWeight: "500", whiteSpace: "nowrap" }}>
            {"Читать разбор "}
            <span style={{ width: "36px", height: "36px", borderRadius: "999px", background: "rgba(255,255,255,.14)", display: "grid", placeItems: "center" }}>
              →
            </span>
          </span>
        </div>
      ...
      <div style={{ marginTop: "16px", borderRadius: "28px", background: "#fff", padding: "clamp(24px,3vw,36px)", display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(280px,1fr))", gap: "24px", alignItems: "center" }}>
        <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
          ...
          {" "}
          <span style={{ fontSize: "15px", lineHeight: "1.5", color: "#565E6B", maxWidth: "420px" }}>
            Разборы возвратов, размерных сеток и примерки — на данных магазинов. Без новостей и без рассылок «мы обновили дизайн».
          </span>
        </div>
        {" "}
        <form onSubmit={v.subscribe} style={{ display: "flex", gap: "10px", flexWrap: "wrap", justifySelf: "end", width: "min(100%,420px)" }}>
          <input className="scp7" required type="email" placeholder="Рабочая почта" style={{ flex: "1", minWidth: "180px", height: "56px", borderRadius: "999px", border: "1px solid rgba(18,20,23,.12)", padding: "0 20px", fontSize: "16px", fontFamily: "'Golos Text',sans-serif", color: "#121417", outline: "none" }} />
          {" "}
          <button className="scp8" type="submit" style={{ height: "56px", padding: "0 24px", borderRadius: "999px", border: "none", background: "#121417", color: "#fff", fontFamily: "'Golos Text',sans-serif", fontSize: "16px", fontWeight: "500", cursor: "pointer", whiteSpace: "nowrap" }}>
            {v.subLabel}
          </button>
        </form>
      </div>
