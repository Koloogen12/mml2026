...
function isStageEnabled(
  ...
  if (stage === 'privacyPolicy') return true;
...
interface WidgetState {
  ...
  // Products that are actually rendered on the current try-on result image.
  // Updated only when a try-on finishes successfully. Used to decide which
  // layer tiles show the "×" clear affordance (we don't want the × to appear
  // the moment a user picks a product — only after the render actually lands).
  ...
  setWornProductIds: (ids: string[]) => void;
  clearTryOn: () => void;
...
export const useWidgetStore = create<WidgetState>((set) => ({
  ...
  selectProduct: (product) =>
    set((state) => {
      ...
      // Up to 5 concurrent garments (outerwear + tops + bottoms + shoes +
      // accessories, one per layer). Raised from 3 so the buyer can try a
      // full outfit head-to-toe.
      if (filtered.length >= 5) return state;
  ...
  deselectProduct: (productId) =>
    set((state) => ({
      ...
      // If the user removes a garment that was worn in the last render, it's
      // no longer "worn" — drop it from wornProductIds so the × disappears
      // (the carousel opens in its slot instead).
      wornProductIds: state.wornProductIds.filter((id) => id !== productId),
    })),

  setWornProductIds: (ids) => set({ wornProductIds: ids }),
  ...
  applySession: (data) => {
    ...
    set({
      ...
      bodyParams: {
        ...
        chest: data.chest || initialBodyParams.chest,
        waist: data.waist || initialBodyParams.waist,
        hip: data.hip || initialBodyParams.hip,
        euSize: data.size || initialBodyParams.euSize,
...
useWidgetStore.subscribe((state, prevState) => {
  ...
  syncTimer = setTimeout(() => {
    ...
    updateSession(sessionToken, {
      ...
      chest: bodyParams.chest,
      waist: bodyParams.waist,
      hip: bodyParams.hip,
      size: bodyParams.euSize,
